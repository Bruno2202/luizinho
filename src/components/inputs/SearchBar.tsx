...
export default function SearchBar({ searchTerm, showResult, setSearchTerm, searchFunctionByTerm, setResult, ...props }: Props) {
                    ...
                    <div className="absolute top-12  z-10 bg-white rounded-md shadow-md w-full max-h-48 overflow-y-auto border border-gray-200">
                        {searchData.map((data, index) => (
                            <p
                                key={data.id || index}
                                onClick={() => {
                                    setSearchTerm(data.name || data.description);
                                    setResult?.(data);
                                    setIsSearchOpen(false);
                                }}
